...
* 회원가입
- 아이디 중복 검사 및 유효성 검사
- 비밀번호 확인 및 Spring Security 모듈에서 제공하는 BCryptPasswordEncoder 클래스를 활용한 비밀번호 암호화
- 이름, 이메일, 전화번호 유효성 검사
* 로그인
- 사용자 코드를 부여하여 관리자, 일반 회원, 탈퇴 회원으로 구분
- 암호화 여부를 확인한 후 로그인 가능
- admin(관리자), test(테스트 아이디)는 암호화를 거치지 않고 임시 비밀번호(**1111**)로 로그인 가능
* 카테고리
- 카테고리별로 10개의 상품만 조회
*상품 상세 보기
- 사이즈와 수량을 선택하지 않으면 장바구니에 담을 수 없음
- 사이즈, 수량, 합계 금액을 실시간으로 변경
* 장바구니
- 상품 삭제 및 총 수량과 합계 금액 확인 가능
- 로그인 후 이용 가능
* 주문/결제 페이지
- 주문자 정보(휴대전화, 주소) 변경 가능
- 주문 상품, 결제 정보 확인 후 결제 수단 선택하여 주문 상세 내역으로 이동(DEFAULT - 결제 완료)
* 주문 상세 내역
- 주문 일자, 주문 번호, 상품 정보 확인 가능
- 주문 취소 가능
- 주문자 이름과 연락처는 마스킹 처리
- 총 주문 금액 확인 후 리뷰 페이지로 이동 가능
* 리뷰 목록
- 사용자 리뷰와 관리자 답글 확인 가능
- 본인의 리뷰만 수정 및 삭제 가능
- 주문자 이름은 마스킹 처리
* 리뷰 등록 페이지
- 주문 정보 조회하여 상단에 주문 정보 표시
- 하나의 상품을 선택하여 리뷰 작성


✔ 관리자 기능
* 관리자 패널
- 관리자 패널에서 총 매출 금액, 총 주문 건수, 총 리뷰 건수, 총 상품 건수 확인 가능
- 각 관리 패널로 이동 가능
* 월별 판매량 및 매출표
- 월별 판매량 및 매출 그래프와 테이블 제공(Chart.js 활용)
* 총 주문 목록
- 사용자 주문 정보, 결제 정보 확인 가능
* 관리자 리뷰 답글
- 사용자 리뷰에 대한 답글 등록, 수정, 삭제 가능
* 총 상품 목록
- 상품 목록 조회 및 상품 등록, 수정, 삭제(UPDATE) 가능
* 관리자 상품 등록
- 상품의 정보 및 이미지 업로드 기능
* 관리자 상품 수정, 삭제
- 상품의 카테고리, 사용여부를 수정(삭제 시 DELETE가 아닌 UPDATE)
* 총 회원 목록
- 사용자의 CODE를 분류 하여 사용자 정보 조회(일반 회원, 탈퇴 회원, 정지 회원 등)
* 카카오맵
- 카카오 API를 활용한 위치 및 로드뷰 제공
<br>

<br><br>
<hr>
AWS SDK를 사용하여 파일을 S3 버킷을 통해 저장 및 관리 할 수 있도록 구현 예정
<br>
