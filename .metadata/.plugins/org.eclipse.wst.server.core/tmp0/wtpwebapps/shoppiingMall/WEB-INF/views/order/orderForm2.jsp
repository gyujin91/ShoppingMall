   	...
   	<script>
		...
		/* document.getElementById('btn').addEventListener('click', function() {
		    ...
		    if (bankTransferCheckbox.checked) {
		        // "무통장 입금" 체크박스가 체크된 상태일 때 insertOrder.do, insertPayment.do 메소드를 호출 및 회원정보 수정
		        if(info == 'omission') {
		    		alert("필수 정보가 누락 되었습니다.");
		    	} else {
		    		paymentMemberUpdate();
			        insertPayment("무통장 입금");
			        insertOrder('${mem_id}');
		    	}		        
		    ...
		    if (bankTransferCheckbox.checked) {
		        ...
		        if (paymentMethodSelect.value !== 'select') {
		            ...
		            if (info == 'omission') {
		                ...
		                paymentMemberUpdate(); // 회원 정보 업데이트
		                insertPayment(paymentMethodSelect.value, "무통장 입금"); // 결제 정보 삽입
		...
		function insertOrder() {
		    ...
		    $.ajax({
		         ...
		         success: function(response) {
		             ...
		             window.location.href = "${path}/order/orderDetail.do?order_no=" + response.order_no;
		         },
		         error: function(xhr, status, error) {
		...
		function insertPayment(labelOption, selectedOption, order_no) {
			
			$.ajax({
				url: '/myapp/payment/insertPayment.do',
				method: 'POST',
				data: {
